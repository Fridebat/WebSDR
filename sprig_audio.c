...
static int set_hwparams(snd_pcm_t *handle, snd_pcm_hw_params_t *params) {
  ...
  err = snd_pcm_hw_params_set_buffer_time_near(handle, params, &mybuffer_time, &dir);
  if (err < 0) {
  ...
  err = snd_pcm_hw_params_set_period_time_near(handle, params, &myperiod_time, &dir);
  if (err < 0) {
...
static void async_callback(snd_async_handler_t *ahandler) {
  ...
  while (avail >= myperiod_size) {
    ...
    if(0) fprintf( stderr, "async_callback : icount = %8d, avail = %12ld \n",
        icount, avail);

    double amp = (icount>>4) % 2 == 0 ? 0.0 : 1.0;

    for (int i = 0; i < NFFT; i++) {
      ringbuffer[wpnt] = 1.0 * samples[i]
                       + 0.2 * 32767.0 *        amp  * sin ( 2.0 * M_PI * (double) (i % 32) / 32.0 )
                       + 0.2 * 32767.0 * (1.0 - amp) * sin ( 2.0 * M_PI * (double) (i % 16) / 16.0 );
      wpnt++;
      wpnt &= 0x3fff;
    }
