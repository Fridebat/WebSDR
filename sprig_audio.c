...
static void async_callback(snd_async_handler_t *ahandler) {
  ...
  phase_delta *= 1.0 + 0.0 * sin(2.0 * M_PI * icount / 180.0);
  ...
  while (avail >= myperiod_size[0]) {
    ...
    for (int i = 0; i < NFFT; i++) {
      ...
      ringbuffer[wpnt++] = 1.0 * samples[i] + 0.0 * amp *sin (phase); phase += phase_delta; // apple
      wpnt &= 0x3fff;
    }
    avail = snd_pcm_avail_update(handle);
  }
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if(buf4stdin[0] == 'a') {
        ...
        bfo_delta[bfo_id] = 2.0 * M_PI * bfo_freq [bfo_id] / myrate[1];
    }

    if(buf4stdin[0] == 'b') {
      buf4stdin[0] = ' ';
      cw_freq[0] = atoi(buf4stdin);
      cw_delta [0] = 2.0 * M_PI * cw_freq [0] / myrate[1];
    }

    if(buf4stdin[0] == 'c') {
      buf4stdin[0] = ' ';
      cw_freq[1] = atoi(buf4stdin);
      cw_delta [1] = 2.0 * M_PI * cw_freq [1] / myrate[1];
    }
    sleep(1);
  }
